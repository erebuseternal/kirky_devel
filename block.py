...
# can multiply these on the right of a normal number and you
# can add them together. That's it. But as soon as you set a leaf
# all of its children get updated. And as soon as all parents of a 
# leaf are set, the leaf gets set as well
class SymbolicNumber:
    ...
    def __init__(self, watcher=None):
        ...
        self.value = 0
        self.multiplier = 1
        self.num_set_parents = 0
        self.children = []
        self.parents = []
        # this is so that Symbolic numbers can let everyone know 
        # when they have been set
        self.watcher = watcher
        self.is_leaf = True
        
    def checkIfSetByParents(self):
        # this looks to see if all of the parents have been set
        if self.num_set_parents == len(self.parents):
            # if they have, we set the child as well
            sum = 0
            for parent in self.parents:
                sum += parent.multiplier * parent.value
            self.Set(self.multiplier * sum)
        
    def Set(self, number):
        if self.symbolic == True:
            self.symbolic = False
            self.value = number
            # now we handle the children by letting them know of this 
            # setting of their parent and making sure they check they 
            # have not been set as well as a result
            for child in self.children:
                child.num_set_parents += 1
                child.checkIfSetByParents()
        else:
            raise Issue('you cannot set an already set SymbolicNumber!')
        
    
    def UnSet(self):
        if self.symbolic == False and len(self.parents) == 0:
            self.symbolic = True
            self.value = 0
            for child in self.children:
                child.num_set_parents -= 1
            ...
            raise Issue('you cannot unset an already unset SymbolicNumber!')
        
    def SetChild(self, number):
        # this is a very special method. It should only be called when 
        # exactly one of the parents is still unset. What it will do is 
        # set that parent to the number that will allow its value to be 
        # the number input
        if self.is_leaf:
            raise Issue('a leaf is not a child!')
        if self.symbolic == False:
            raise Issue('you cannot set an already set Symbolic Number')
        if len(self.parents) - 1 > self.num_set_parents:
            raise Issue('you cannot call set child if it has more than one unset parent!')
        unset_parent = None
        dif = number
        for parent in self.parents:
            if parent.symbolic == True:
                unset_parent = parent
            else:
                dif -= parent.multiplier * parent.value
        # we get the value we need to set on the unset parent
        value = dif / unset_parent.multiplier
        # and we set the unset parent
        unset_parent.Set(value)
        # and that's it
    
    def AddParent(self, parent):
        self.parents.append(parent)
        self.is_leaf = False
        if parent.symbolic == False:
    ...
    def __rmul__(self, other):
        new_number = SymbolicNumber()
        new_number.multiplier = other
        new_number.parents.append(self)
        self.children.append(new_number)
        new_number.is_leaf = False
        if self.symbolic == False:
            new_number.num_set_parents += 1
            new_number.checkIfSetByParents()
        return new_number
        
    def __add__(self, other):
        # we need to first create a new SymbolicNumber
        # this we need to be able to reach from its forebearers
        new_number = SymbolicNumber()
        self.children.append(new_number)
        other.children.append(new_number)
        new_number.parents.append(self)
        new_number.parents.append(other)
        new_number.is_leaf = False
        if self.symbolic == False:
            new_number.num_set_parents += 1
    ...
    def __iadd__(self, other):
        # here we need to add in a new parent
...
# for any kirchhoff problem there is always going to be a number 
# of vectors which is just the number of columns from our 
# original matrix. So we shall keep these in mind!

class Vertex:

    def __init__(self, position, num_vectors):
        self.position = position
        # the cut is a list of lists. Each inner list has two spaces 
        # the first is for a vector leaving, the second is for a 
        # vector entering. Each interior list is there for one vector 
        self.edges = [[None,None]] * num_vectors

    def addEdge(self, edge, head):
        vector_id = edge.vector_id
        # now we make sure there isn't already an edge in the place 
        # where it belongs. If there, by virtue of how blocks are built
        # then this edge is the edge already there
        if head:
